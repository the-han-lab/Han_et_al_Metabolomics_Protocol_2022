...
"""
Get p-values and corrected p-values for fold change values in a set of colonizations (e.g., Bt mono-colonized)
relative to the germ-free values in the same sample type (e.g., urine)

python3 calculate_stats_sig_with_correction.py -f fold_change_matrix_mode_collapsed.txt --metadata metadata.txt --sample_type urine
"""

def calculate_pvalues(fc_matrix,
                      colonizations,
                      sample_type,
                      sample_or_media_type='sample_type',
                      colonization_or_bacteria='colonization',
                      germ_free_str='germ-free'):
    grouped = fc_matrix.groupby([colonization_or_bacteria])

    control_group = fc_matrix[(fc_matrix[colonization_or_bacteria] == germ_free_str) & (fc_matrix[sample_or_media_type] == sample_type)]
    # print('\nControl group:\n')
    # print(control_group)
    ...
    for metabolite in fc_matrix.columns:
        if metabolite in ['experiment_type', colonization_or_bacteria, sample_or_media_type]:
            continue
    ...
    return pd.DataFrame({
        colonization_or_bacteria: df_colonizations,
...
if __name__ == "__main__":
    ...
    fc_matrix = fc_matrix \
        .join(metadata[['experiment_type','sample_type', 'colonization']])
    ...
    df.to_excel(f"corrected_pvalues_{sample_type}.xlsx")
