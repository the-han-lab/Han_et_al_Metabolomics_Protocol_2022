...
"""
This script searches the extracted MS2 spectral data from biological samples (.msp file) 
against the user-provided MS2 spectral library (.msp file). 

If the MS2 spectra from biological samples have been collected using the C18 positive method, 
select the MS2 spectral library generated by the same method. 

The script takes an input folder containing one or multiple spectral .msp files from biological samples, and generates  two types of output files: 

1) An excel spreadsheet summarizing the MS2 similarity search result for each metabolite at the collision energy that it has been collected. 

Expect spectral patterns to differ for the same metabolites collected at different collision energies. 

This document provides the top five matches based on the user-provided MS2 library, ranked based on their similarity scores.

2) A PDF file of mirror plots visually displaying the spectral patterns between each metabolite 
and its library match at the collision energy that it has been collected. 

This document provides the identical top five matches as shown in the summary excel spreadsheet, 
ranked based on their similarity scores. 

Expect to generate multiple PDF files corresponding to the multiple metabolites that are matched to the MS2 library.
"""

"""
Example command:

python3 library_ms2_similarity_search.py input/example_c18negative_QE_input \
    --library_msp input/QE_c18negative_library.msp \
    --mode c18negative \
    --library ../data_analysis/input/mz-rt_library.xlsx
"""
